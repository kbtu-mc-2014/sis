...
public class GameLoopThread extends Thread {
	...
	private final static int MAX_FPS = 30; // How many times per second the game
											// should be updated, drawn?

	private final static int MAX_FRAME_SKIPS = 95; // Maximum number of frames to
													// be skipped
			...
			try {
				...
				synchronized (surfaceHolder) {
					...
					while (sleepTime < 0 && framesSkipped < MAX_FRAME_SKIPS) {
						...
						// We need to catch up, so we update without drawing the
						// game to the screen.

						this.game.Update(this.gameTime);

						sleepTime += FRAME_PERIOD; // Add FRAME_PERIOD to check
